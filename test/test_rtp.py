...
def test_calc_furthest_full_range():
    assert rtp.calc_furthest_full_range(rtp.TimeInterval(0,1), 
                                        rtp.TimeInterval(1,2),1,10) == 5
    # Test varying baud length
    assert rtp.calc_furthest_full_range(rtp.TimeInterval(0,1), 
                                        rtp.TimeInterval(1,2),0.1,10) == 5
    
    # Test varying starttime for reception
    assert rtp.calc_furthest_full_range(rtp.TimeInterval(0,1), 
                                        rtp.TimeInterval(10,12),1,10) == 55
    
    # Test varying endtime for reception
    assert rtp.calc_furthest_full_range(rtp.TimeInterval(0,1), 
                                        rtp.TimeInterval(1,20),1,10) == 95
    
    # Test displacement in time
    assert rtp.calc_furthest_full_range(rtp.TimeInterval(10,11), 
                                        rtp.TimeInterval(11,12),1,10) == 5
